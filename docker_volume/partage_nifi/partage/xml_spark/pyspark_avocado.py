...
# Lire le fichier CSV
df = spark.read.csv(f'/projet_elasticsearch_kibana/raw_avocado/avocado_5.csv', header=True, inferSchema=True)
df.show()
...
df.write.csv(r'/projet_elasticsearch_kibana/stagging_avocado/avocado_cleaned_5.csv', header=True)

# Chargement pour indexer
df_index = spark.read.option("header", True) \
.option("inferSchema" , "true").csv(f"/projet_elasticsearch_kibana/stagging_avocado/avocado_cleaned_5.csv")
